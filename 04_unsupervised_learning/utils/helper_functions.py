...
def show_images(num_images):
    ...
    if num_images % 10 == 0 and num_images <= 100:
        for digit_num in range(0, num_images):
            ...
            mat_data = X.iloc[digit_num].to_numpy().reshape(
                28, 28)  # reshape images
...
def show_images_by_digit(digit_to_see):
    ...
    if digit_to_see in list(range(10)):
        ...
        for digit_num in range(0, 50):
            ...
            mat_data = X.iloc[indices[0][digit_num]
                              ].to_numpy().reshape(28, 28)  # reshape images
            plt.imshow(mat_data)  # plot the data
            plt.xticks([])  # removes numbered labels on x-axis
            plt.yticks([])  # removes numbered labels on y-axis
    else:
...
def plot_components(X, y):
    ...
    for i in range(X.shape[0]):
        plt.text(X[i, 0], X[i, 1], str(y[i]),
                 color=plt.cm.Set1(y[i]), fontdict={'size': 15})
...
def scree_plot(pca):
    ...
    for i in range(num_components):
        ax.annotate(r"%s%%" % (
            (str(vals[i]*100)[:4])), (ind[i]+0.2, vals[i]), va="bottom", ha="center", fontsize=12)
...
def plot_component(pca, comp):
    ...
    if comp <= len(pca.components_):
        mat_data = np.asmatrix(pca.components_[comp]).reshape(
            28, 28)  # reshape images
        plt.imshow(mat_data)  # plot the data
        plt.xticks([])  # removes numbered labels on x-axis
        plt.yticks([])  # removes numbered labels on y-axis
